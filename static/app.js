...
function renderLogin(mode = "signin") {
  const title = mode === "forgot" ? "ลืมรหัสผ่าน" : "เข้าสู่ระบบ";
  root().innerHTML = "";
  root().append(
    h(`<div class="center-wrap">
      <div class="card auth-card auth-card-sm">
        <div class="card-head">
          <h1 class="auth-title">${icon("package")}${title}</h1>
        ...
        <div class="card-body">
          <form id="auth-form">
            ...
            ${
              mode === "signin"
                ? `<div class="field"><label>รหัสผ่าน</label><input class="input" name="password" type="password" placeholder="••••••••" required /></div>`
                : ""
            }
          ...
          <div class="auth-links">
            ${
              mode === "signin"
                ? `<button class="link muted-link" id="to-forgot">ลืมรหัสผ่าน?</button>`
                : `<button class="link muted-link" id="to-signin">← กลับไปเข้าสู่ระบบ</button>`
            }
          </div>
        </div>
      </div>
...
function renderReset() {
  ...
  root().append(
    h(`<div class="center-wrap"><div class="card auth-card auth-card-sm"><div class="card-body">
      <h1 class="auth-title">${icon("package")}ตั้งรหัสผ่านใหม่</h1>
...
function renderDonate() {
  ...
  const paintGate = (err = "") => {
    root().innerHTML = "";
    root().append(
      h(`<div class="center-wrap"><div class="card auth-card auth-card-sm">
        <div class="card-head text-center">
          <img class="logo-round" src="/static/logo.png" alt="Saint Martin Foundation" />
          <h1 class="auth-title text-primary">ยืนยันสาขา</h1>
          <p class="auth-sub">กรุณากรอกรหัสสาขาของคุณเพื่อเข้าสู่แบบฟอร์ม</p>
        </div>
        <div class="card-body">
        <form id="gate">
          <div class="field"><label>รหัสสาขา</label><input class="input" name="code" value="${preset}" placeholder="เช่น 15005" autofocus required /></div>
  ...
  const paintSuccess = () => {
    root().innerHTML = "";
    root().append(
      h(`<div class="center-wrap"><div class="card auth-card auth-card-sm"><div class="card-body success-box">
        <div class="success-check">✓</div>
        <h2>บันทึกสำเร็จ!</h2>
        <p class="muted">ข้อมูลการรับของบริจาคถูกบันทึกเรียบร้อยแล้ว</p>
        <button class="btn" id="again">${icon("plus")} บันทึกรายการใหม่</button>
      </div></div></div>`)
    );
  ...
  const paintForm = () => {
    ...
    root().innerHTML = "";
    root().append(
      h(`<div class="min-h-screen">
        <header class="header">
          <div class="container donate-head">
            <img class="logo-round sm" src="/static/logo.png" alt="Saint Martin Foundation" />
            <h1>แบบฟอร์มบันทึกรับของบริจาค</h1>
          </div>
        </header>
        <main class="container donate-main">
        <div class="card">
          <div class="card-head">
            <h2 class="auth-title">${icon("plus")}บันทึกรับของบริจาค</h2>
          </div>
          <div class="card-body">
            <div class="branch-chip"><span class="text-primary">${confirmed.code}</span><span class="muted">— ${confirmed.name}</span></div>
            ${donationFormHtml({
              date: todayISO(),
...
function donationFormHtml(opts) {
  ...
  return `<form data-donation>
    <div class="form-grid">
      ...
      <div class="field"><label>จำนวนชิ้น *</label><input class="input" name="pieces" type="number" min="0" placeholder="0" required /></div>
      <div class="field"><label>จำนวนตะกร้า</label><input class="input" name="baskets" type="number" min="0" placeholder="0" /></div>
      ...
      <div class="field"><label>น้ำหนักรวม (กก.)</label><input class="input" name="weightKg" type="number" min="0" step="0.01" placeholder="0.00" /></div>
    ...
    <button class="btn btn-full" type="submit" style="margin-top:1rem">${icon("plus")} บันทึก</button>
  </form>`;
}
...
function switchTab(tab) {
  ...
  $("#month-nav").style.display = tab === "entry" ? "flex" : "none";
  const renderers = {
    entry: renderEntry,
    list: renderList,
...
async function loadMonthSummary() {
  ...
  el.innerHTML = `
    ...
    <div class="grid-3">
      <div class="card stat"><div class="stat-icon">${icon("package")}</div><div><p class="muted">จำนวนครั้งที่รับ</p><p class="stat-value">${fmtNum(t.totalRecords)} ครั้ง</p></div></div>
      <div class="card stat"><div class="stat-icon">${icon("list")}</div><div><p class="muted">จำนวนชิ้นรวม</p><p class="stat-value">${fmtNum(t.totalPieces)} ชิ้น</p></div></div>
      <div class="card stat"><div class="stat-icon">${icon("bar")}</div><div><p class="muted">น้ำหนักรวม</p><p class="stat-value">${fmtNum(t.totalWeight, 2)} กก.</p></div></div>
    </div>`;
}
...
async function renderEntry() {
  ...
  main.innerHTML = `<div class="space-y">
    ...
    <div class="card"><div class="card-head"><h2 class="auth-title">${icon("plus")}บันทึกรับของบริจาค</h2></div>
    <div class="card-body">
      ${donationFormHtml({})}
    </div></div>
  </div>`;
...
async function renderBranchReport() {
  ...
  $("#main").innerHTML = `<div class="card"><div class="card-body">
    <div class="flex-between wrap">
      <h2 style="margin:0">สรุปตามสาขา</h2>
      <div class="flex-between">
        <div class="month-nav">
          <button class="btn btn-outline btn-icon" id="br-prev">${icon("left")}</button>
          <span>${data.label}</span>
          <button class="btn btn-outline btn-icon" id="br-next">${icon("right")}</button>
        </div>
        <a class="btn btn-outline btn-sm" href="/api/export/branches.xlsx?year=${y}&month=${m}">ส่งออก Excel</a>
      </div>
    </div>
    <div class="grid-3" style="margin:1rem 0">
      <div class="card stat"><div class="stat-icon">${icon("store")}</div><div><p class="muted">จำนวนสาขา</p><p class="stat-value">${fmtNum(data.totals.branchCount || 0)}</p></div></div>
      <div class="card stat"><div class="stat-icon">${icon("package")}</div><div><p class="muted">จำนวนครั้งรวม</p><p class="stat-value">${fmtNum(data.totals.totalRecords)}</p></div></div>
      <div class="card stat"><div class="stat-icon">${icon("list")}</div><div><p class="muted">จำนวนชิ้นรวม</p><p class="stat-value">${fmtNum(data.totals.totalPieces)}</p></div></div>
    </div>
    ${
      data.rows.length
...
function renderNotFound() {
  root().innerHTML = "";
  root().append(
    h(`<div class="center-wrap bg-muted">
      <div class="text-center">
        <h1 class="notfound-code">404</h1>
        <p class="muted" style="font-size:1.15rem">Oops! Page not found</p>
