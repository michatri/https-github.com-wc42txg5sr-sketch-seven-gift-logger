...
| ข้อมูลสาขา | 43 สาขาจากระบบจริง, เพิ่มสาขา, แก้ผู้ติดต่อ |
| LINE | กลุ่มแจ้งเตือนแบบข้อความเต็ม+รูป หรือข้อความย่อ «รับของ» |

สาขาตั้งต้นครบชุดเดียวกับเว็บออนไลน์ เช่น หมู่บ้านเมืองเอก-รังสิต (`15005`), สี่แยกปทุมธานี (`11583`), เทศบาล 10 สามโคก ฯลฯ รวมสาขาที่เคยบันทึกจริง เช่น ชุมชนบางกระดีสายใน

ติดตั้งแล้วจะนำเข้าข้อมูลจาก saintmarkpathum.com อัตโนมัติ: **3,129 รายการรับของ**, ผู้ติดต่อสาขา, กลุ่ม LINE 5 กลุ่ม และรูป 28 ไฟล์ (ดาวน์โหลดตอนติดตั้ง)
...
.venv/bin/python scripts/import_production.py   # นำเข้าข้อมูลจริง + รูป
...
- ข้อมูลรายการอยู่ใน `data/import/snapshot.json` (ดึงจากระบบออนไลน์) และถูกนำเข้า SQLite `data/gift_logger.db` ตอนติดตั้ง
- รูปเก็บที่ `data/photos/` (ดาวน์โหลดจาก storage ของระบบเดิมตอนติดตั้ง — เครื่องต้องออกเน็ตได้)
